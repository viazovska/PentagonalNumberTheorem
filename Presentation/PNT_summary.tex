...
\item \textbf{Punchline:} the \emph{bijective} PNT closed one-shot where it was guided
      (SlidesInput, 1 run); the leaner word-only OnlyWordsInput took a 2nd run and still carries
      1 \texttt{sorry}; the \emph{analytic} JTP route needed $\sim$6 iterative runs --- two spent
      only \emph{planning} --- yet the whole Aristotle effort ($\approx\$220$--$880$) still came in
