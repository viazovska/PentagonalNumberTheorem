...
\midrule
Total lines        & \textbf{1{,}350}      & \textbf{728}        & \textbf{460} \\
Files              & 5                     & 3                   & 3 \\
Status             & \alert{\texttt{sorry}-free} & \alert{\texttt{sorry}-free} & 2 \texttt{sorry}s \\
\bottomrule
\end{tabular}
\medskip
...
\item \textbf{\texttt{SlidesInput} (728):} \emph{one} map \texttt{franklinMap} (ops A/B
      ...
      tighter packaging.
\item \textbf{\texttt{OnlyWordsInput} (460):} leanest --- rides mathlib's
      \texttt{Finset.sum\_involution} and a \texttt{topRun} helper. Closes the cancellation
      argument but leaves the involutivity and fixed-point lemmas as \texttt{sorry}.
\end{itemize}
\medskip
\footnotesize\emph{Same mathematics, $\sim$3$\times$ length spread: more guidance / more library
reuse $\Rightarrow$ shorter code, but the shortest run did not fully close.}
\end{frame}
...
\midrule
Aristotle (\emph{est.}) & \multicolumn{4}{c}{$\approx$\,50--130\,M total --- \alert{$\sim$8--12 runs} total} \\
\bottomrule
\end{tabular}
\medskip
...
\item \textbf{Aristotle (est.\ --- no token logs; from run counts):}
      OnlyWordsInput 1 run / 460\,ln / 2 \texttt{sorry}s; SlidesInput 1 run / 728\,ln / closed;
      qSeries+JTP \alert{6 documented runs} (2 spent only on feasibility/cost analysis) / 1.7\,k\,ln / closed.
      Total $\sim$8--12 runs, $\approx$50--130\,M tok, $\approx\$200$--$800$, $\approx$4--20\,kWh.
...
\item \textbf{Punchline:} the \emph{bijective} PNT closed in \emph{one} Aristotle run each (cheap,
      one-shot); the \emph{analytic} JTP route needed $\sim$6 iterative runs --- two spent only
      \emph{planning} --- yet the whole Aristotle effort ($\approx\$200$--$800$) still came in
      \emph{below} the interactive exploration around it ($\approx\$1{,}500$). Easy proofs are
      one-shot; hard ones iterate --- but generating proofs stays cheaper than talking about them.
\end{itemize}
\end{frame}
