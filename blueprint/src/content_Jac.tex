...
\texttt{Series/qSeries.lean}. The proof realised in Lean follows Heine's
...
three textbook sketches above; it is documented in
...
\texttt{qSeries\_aristotle}, split into the modules
...
\texttt{QSeries/JTP\_Core.lean},
\texttt{QSeries/JacobiTripleProduct.lean}, and
\texttt{QSeries/PentagonalNumber.lean}, all under the namespace
\texttt{qSeries}. The $q$-Pochhammer / $q$-binomial foundations
(Section~\ref{sec:qPochhammer-formalized}) are shared with the monolithic
\texttt{Series/qSeries.lean}; everything below builds on them.
...
\begin{definition}[Triple product and bilateral series]\label{def:jacobiProd}\lean{qSeries.jacobiProd}\leanok\uses{def:qPochhammerInf}
