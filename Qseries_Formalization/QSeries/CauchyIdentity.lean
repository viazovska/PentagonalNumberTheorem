...
theorem aux_cauchy_norm_mul_lt_one {q z : ℂ} (hq : ‖q‖ < 1) (hz : ‖z‖ < 1) :
    ‖q * z‖ < 1 := by
  rw [norm_mul]
  nlinarith [norm_nonneg q, norm_nonneg z]

/-- If `‖q‖ < 1` and `‖z‖ < 1` then `‖q ^ n * z‖ < 1` for every `n`. -/
theorem aux_cauchy_norm_pow_mul_lt_one {q z : ℂ} (hq : ‖q‖ < 1) (hz : ‖z‖ < 1) (n : ℕ) :
    ‖q ^ n * z‖ < 1 := by
...
theorem cauchyCoeff_bounded {q : ℂ} (hq : ‖q‖ < 1) (a : ℂ) :
  ...
  have h : Tendsto (fun n => ‖cauchyCoeff a q n‖) atTop
    ...
    ((tendsto_qPochhammer hq).div (tendsto_qPochhammer hq)
      (qPochhammerInf_z_q_ne_zero hq hq)).norm
...
theorem cauchy_functional_eq_G (a z : ℂ) {q : ℂ} (hq : ‖q‖ < 1) (hz : ‖z‖ < 1) :
  ...
  have hzq_ne : qPochhammerInf (z * q) q ≠ 0 :=
    qPochhammerInf_z_q_ne_zero (aux_cauchy_norm_mul_lt_one hz hq) hq
  have hone_sub_z_ne : (1 : ℂ) - z ≠ 0 := fun h => by
    rw [show z = 1 from by linear_combination -h] at hz; norm_num at hz
  rw [qPochhammerInf_recursion (z := z) hq, qPochhammerInf_recursion (z := a * z) hq]
  field_simp
...
private theorem aux_cauchy_hasSum_shift_diff {d : ℕ → ℂ} {z u S : ℂ} (hd0 : d 0 = 1)
...
private theorem hasSum_one_sub_mul_F {a q z : ℂ} (hq : ‖q‖ < 1) (hz : ‖z‖ < 1) :
  ...
  simpa using aux_cauchy_hasSum_shift_diff (u := 1) (cauchyCoeff_zero a q)
    (cauchy_summable hq hz).hasSum
...
private theorem hasSum_one_sub_az_mul_F_qz {a q z : ℂ} (hq : ‖q‖ < 1) (hz : ‖z‖ < 1) :
  ...
  have hd : HasSum (fun n : ℕ => cauchyCoeff a q n * q ^ n * z ^ n)
    ...
    simpa [mul_pow, mul_assoc] using
      (cauchy_summable hq (aux_cauchy_norm_mul_lt_one hq hz)).hasSum
  simpa [mul_assoc] using
    aux_cauchy_hasSum_shift_diff (u := a) (d := fun n => cauchyCoeff a q n * q ^ n)
      (by simp) hd
...
theorem cauchy_functional_eq_F {a q z : ℂ} (hq : ‖q‖ < 1) (hz : ‖z‖ < 1) :
  ...
  rw [hfunc] at hL
  exact hL.unique (hasSum_one_sub_az_mul_F_qz hq hz)
...
theorem iterated_functional_eq_disc {H : ℂ → ℂ} {a q : ℂ} (hq : ‖q‖ < 1)
  ...
  | zero => simp
  | succ n ih =>
      have key := hH _ (aux_cauchy_norm_pow_mul_lt_one hq hz n)
...
theorem qBinom_infinite_thm (a z q : ℂ) (hq : ‖q‖ < 1) (hz : ‖z‖ < 1) :
  ...
  have hH_fe : ∀ w, ‖w‖ < 1 → (1 - w) * H w = (1 - a * w) * H (q * w) := by
    intro w hw
    rw [hH_disc w hw, hH_disc _ (aux_cauchy_norm_mul_lt_one hq hw)]
    exact cauchy_functional_eq_F hq hw
  have key : ∀ n : ℕ,
      F * qPochhammer z q n = H (q ^ n * z) * qPochhammer (a * z) q n := by
    rw [hF_def, ← hH_disc z hz]
    exact iterated_functional_eq_disc hq hH_fe hz
  have hH_lim : Tendsto (fun n => H (q ^ n * z)) atTop (𝓝 1) :=
    (tendsto_F_qpow hq hz).congr fun n => (hH_disc _ (aux_cauchy_norm_pow_mul_lt_one hq hz n)).symm
