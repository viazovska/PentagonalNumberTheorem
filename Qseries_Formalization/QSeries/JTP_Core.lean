...
theorem qPochhammerInf_eq_mul {q : ℂ} (hq : ‖q‖ < 1) (n : ℕ) :
  ...
  rw [qPochhammerInf_shift_div hq n, mul_div_cancel₀ _ (qPochhammer_q_q_ne_zero hq n)]
...
theorem qPochhammerInf_prod_hasSum {q z : ℂ} (hq : ‖q‖ < 1) (hz : ‖z‖ < 1) :
  ...
  rw [qPochhammerInf_shift_div hq n]
  ring
...
def aux_core_diagEquiv : (ℕ × ℕ) ⊕ (ℕ × ℕ) ≃ ℕ × ℕ where
...
theorem aux_core_hasSum_split_diag {f : ℕ × ℕ → ℂ} {a b : ℂ}
    (h1 : HasSum (fun p : ℕ × ℕ => f (p.2 + p.1, p.2)) a)
    (h2 : HasSum (fun p : ℕ × ℕ => f (p.2, p.2 + (p.1 + 1))) b) :
    HasSum f (a + b) :=
  aux_core_diagEquiv.hasSum_iff.mp (HasSum.sum (f := f ∘ aux_core_diagEquiv) h1 h2)

/-- Splitting a summable family over `ℕ × ℕ` along the diagonal:
$\sum_p f(p) = \sum_k \sum_m f(m+k, m) + \sum_l \sum_n f(n, n+l+1)$. -/
theorem aux_core_tsum_split_diag {f : ℕ × ℕ → ℂ} (hf : Summable f) :
    ∑' p : ℕ × ℕ, f p =
      (∑' k : ℕ, ∑' m : ℕ, f (m + k, m)) + ∑' l : ℕ, ∑' n : ℕ, f (n, n + (l + 1)) := by
  have hc : Summable (f ∘ aux_core_diagEquiv) := aux_core_diagEquiv.summable_iff.mpr hf
  ...
  rw [(aux_core_hasSum_split_diag hl.hasSum hr.hasSum).tsum_eq, hl.tsum_prod, hr.tsum_prod]

/-! ### The Jacobi triple product from two `HasSum` inputs -/

/-- Cancellation `z ^ (m + k) * (z⁻¹) ^ m = z ^ k` for `z ≠ 0`. -/
theorem aux_core_pow_add_mul_inv_pow {z : ℂ} (hz : z ≠ 0) (m k : ℕ) :
    z ^ (m + k) * z⁻¹ ^ m = z ^ k := by
  rw [pow_add, mul_right_comm, ← mul_pow, mul_inv_cancel₀ hz, one_pow, one_mul]

/-- Cancellation `z ^ n * (z⁻¹) ^ (n + j) = (z⁻¹) ^ j` for `z ≠ 0`. -/
theorem aux_core_pow_mul_inv_pow_add {z : ℂ} (hz : z ≠ 0) (n j : ℕ) :
    z ^ n * z⁻¹ ^ (n + j) = z⁻¹ ^ j := by
...
`z ^ k * q ^ (k choose 2)`. -/
theorem jtp_diag_nonneg {q z : ℂ} (hq : ‖q‖ < 1) (hz' : z ≠ 0) (k : ℕ) :
    HasSum (fun m : ℕ =>
      q ^ (m + k).choose 2 * z ^ (m + k) * qPochhammerInf (q * q ^ (m + k)) q *
        (q ^ m.choose 2 * q ^ m * z⁻¹ ^ m / qPochhammer q q m)) (z ^ k * q ^ k.choose 2) := by
  refine ((cauchy_coeff_nonneg hq k).mul_left (z ^ k)).congr_fun fun m => ?_
  rw [div_eq_mul_inv, div_eq_mul_inv, ← aux_core_pow_add_mul_inv_pow hz' m k]
  ring

/-- The `z⁻¹ ^ (l + 1)` diagonal of the Jacobi triple product double series sums to
`z⁻¹ ^ (l + 1) * q ^ (l + 2 choose 2)`. -/
theorem jtp_diag_neg {q z : ℂ} (hq : ‖q‖ < 1) (hz' : z ≠ 0) (l : ℕ) :
    HasSum (fun n : ℕ =>
      q ^ n.choose 2 * z ^ n * qPochhammerInf (q * q ^ n) q *
        (q ^ (n + (l + 1)).choose 2 * q ^ (n + (l + 1)) * z⁻¹ ^ (n + (l + 1)) /
          qPochhammer q q (n + (l + 1)))) (z⁻¹ ^ (l + 1) * q ^ (l + 2).choose 2) := by
  refine ((cauchy_coeff_neg hq l).mul_left (z⁻¹ ^ (l + 1))).congr_fun fun n => ?_
  rw [div_eq_mul_inv, div_eq_mul_inv, ← aux_core_pow_mul_inv_pow_add hz' n (l + 1)]
  ring
...
along the diagonal reduces everything to `jtp_diag_nonneg` / `jtp_diag_neg`. -/
theorem jtp_of_hasSum {q z : ℂ} (hq : ‖q‖ < 1) (hz' : z ≠ 0)
  ...
  rw [hfubini, aux_core_tsum_split_diag hprod]
  congr 1
  · exact tsum_congr fun k => (jtp_diag_nonneg hq hz' k).tsum_eq
  · exact tsum_congr fun l => (jtp_diag_neg hq hz' l).tsum_eq
...
theorem jacobiTripleProduct_annulus {q z : ℂ} (hq : ‖q‖ < 1)
  ...
  jtp_of_hasSum hq hz' (qPochhammerInf_prod_hasSum hq hz) (euler_second_at_qoz hq hzq hz')
