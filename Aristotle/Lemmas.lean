...
following the source document "Pentagonal Number Theorem" by JC, PM, MV.
-/
...
/-! ## Section 2.1: Disjointness and covering (Lemmas 11, 12) -/
/-
**Lemma 11 (Source)**: The sets `𝒫_α(n)`, `𝒫_β(n)`, `𝒫_special(n)` are pairwise disjoint.
This states `𝒫_α(n) ∩ 𝒫_β(n) = ∅`.
-/
theorem DPalpha_inter_DPbeta (n : ℕ) : DPalpha n ∩ DPbeta n = ∅ := by
  -- By definition of $DPalpha$ and $DPbeta$, their intersection is empty.
  ext S
  simp [DPalpha, DPbeta];
  grind
/-
**Lemma 11 (Source)**: `𝒫_α(n) ∩ 𝒫_special(n) = ∅`.
-/
theorem DPalpha_inter_DPspecial (n : ℕ) : DPalpha n ∩ DPspecial n = ∅ := by
  unfold DPalpha DPspecial;
  grind
/-
**Lemma 11 (Source)**: `𝒫_β(n) ∩ 𝒫_special(n) = ∅`.
-/
theorem DPbeta_inter_DPspecial (n : ℕ) : DPbeta n ∩ DPspecial n = ∅ := by
  simp [DPbeta, DPspecial];
  grind
/-
**Lemma 12 (Source)**: `𝒫(n) = 𝒫_α(n) ∪ 𝒫_β(n) ∪ 𝒫_special(n)`.
-/
theorem DP_eq_union (n : ℕ) : DP n = DPalpha n ∪ DPbeta n ∪ DPspecial n := by
  -- To prove equality of finite sets, we show each set is a subset of the other.
  apply Finset.ext
  intro S
  simp [DPalpha, DPbeta, DPspecial];
  grind
/-! ## Section 2.1: Properties of SmkSet and SpkSet (Definition 13) -/
/-
**Definition 13 (Source)**: For `k ≥ 1`, `SmkSet k` has `k` elements.
-/
theorem SmkSet_card (k : ℕ) (hk : 1 ≤ k) : (SmkSet k).card = k := by
  unfold SmkSet; rw [ Nat.card_Icc ] ; omega;
/-
**Definition 13 (Source)**: For `k ≥ 1`, `SmkSet k` sums to `(3k²−k)/2`.
-/
...
/-
**Definition 13 (Source)**: For `k ≥ 1`, `SpkSet k` has `k` elements.
-/
theorem SpkSet_card (k : ℕ) (hk : 1 ≤ k) : (SpkSet k).card = k := by
  -- By definition of `SpkSet`, we know that its cardinality is `k`.
  simp [SpkSet];
  rw [ two_mul, Nat.add_sub_cancel ]
/-
**Definition 13 (Source)**: For `k ≥ 1`, `SpkSet k` sums to `(3k²+k)/2`.
-/
...
/-! ## Section 2.1: Special partitions (Lemma 14) -/
/-
**Lemma 14, non-pentagonal case (Source)**: If `n ≥ 1` is not a generalized pentagonal number,
then `𝒫_special(n) = ∅`.
-/
theorem DPspecial_empty_of_nonpent (n : ℕ) (hn : 1 ≤ n)
    ...
    DPspecial n = ∅ := by
      -- Suppose S ∈ DPspecial n with n ≥ 1. Since n ≥ 1, S ≠ ∅, so S is nonempty with base b, slope s, max m, slope set D.
      by_contra h_nonempty
      obtain ⟨S, hS⟩ : ∃ S ∈ DPspecial n, S ≠ ∅ := by
        obtain ⟨ S, hS ⟩ := Finset.nonempty_of_ne_empty h_nonempty; use S; simp_all +decide [ DPspecial ] ;
        rintro rfl; simp_all +decide [ DP ] ;
        grind +splitImp;
      -- Since S is nonempty and in DPspecial, it must satisfy the conditions for being special.
      obtain ⟨b, m, s, hb, hm, hs, hD⟩ : ∃ b m s, partBase S = b ∧ partMax S = m ∧ partSlope S = s ∧ partMax S - partSlope S + 1 ≤ partBase S ∧ (partBase S = partSlope S ∨ partBase S = partSlope S + 1) := by
        unfold DPspecial at hS; aesop;
      -- Since S is nonempty and in DPspecial, it must satisfy the conditions for being special. Therefore, S = {b, b+1, ..., m}.
      have hS_eq : S = Finset.Icc b m := by
        ...
        · intro x hx; have := Finset.mem_Icc.mp hx; simp_all +decide [ partBase, partMax ] ;
          ...
          · omega;
      -- Since S is nonempty and in DPspecial, it must satisfy the conditions for being special. Therefore, m = b + s - 1.
      ...
      -- Since S is nonempty and in DPspecial, it must satisfy the conditions for being special. Therefore, n = (b + m) * s / 2.
      ...
      · simp_all +decide [ ← two_mul, Nat.mul_div_cancel_left ];
        exact h1 1 le_rfl ( by norm_num; linarith [ show b = 1 from by { unfold DPspecial at hS; aesop } ] );
...
/-
**Lemma 14, pentagonal minus case (Source)**: If `n = (3k²−k)/2` for `k ≥ 1`,
then `𝒫_special(n) = {S_{−k}}`.
-/
theorem DPspecial_pent_minus (n k : ℕ) (hk : 1 ≤ k) (hn : 2 * n = 3 * k ^ 2 - k) :
    DPspecial n = {SmkSet k} := by
      unfold DPspecial;
      ext S;
      constructor;
      · simp +zetaDelta at *;
        ...
        · -- Since S is nonempty and partBase S = partSlope S or partBase S = partSlope S + 1, we have S = SmkSet k.
          have hS_eq_SmkSet : S = Finset.Icc (partBase S) (partMax S) := by
            ...
            · intros m hm
              ...
              · grind;
          -- Since S is nonempty and partBase S = partSlope S or partBase S = partSlope S + 1, we have S.sum id = (3 * partBase S ^ 2 - partBase S) / 2 or S.sum id = (3 * (partBase S - 1) ^ 2 + (partBase S - 1)) / 2.
          ...
          -- Since S is nonempty and partBase S = partSlope S or partBase S = partSlope S + 1, we have S.sum id = n.
          have hS_sum_eq_n : S.sum id = n := by
            exact DP_sum n S hS;
          rcases hS_sum with h | h <;> rw [ h ] at hS_sum_eq_n;
          · -- Since $2n = 3k^2 - k$, we have $3 * partBase S ^ 2 - partBase S = 3 * k ^ 2 - k$.
            ...
            -- Since $3 * partBase S ^ 2 - partBase S = 3 * k ^ 2 - k$, we have $partBase S = k$.
            have h_partBase_eq_k : partBase S = k := by
              ...
              · nlinarith only [ hk ];
            -- Since $partBase S = k$, we have $partMax S = 2k - 1$.
          ...
          · -- Since $2n = 3k^2 - k$, we have $k = partBase S - 1$.
      ...
      · simp +zetaDelta at *;
        ...
        · -- Let's calculate the values of `partBase`, `partMax`, and `partSlope` for `SmkSet k`.
          ...
          have h_slope : partSlope (SmkSet k) = k := by
            -- By definition of `consecutiveTopRun`, we know that `consecutiveTopRun (SmkSet k) (2 * k - 1) = k`.
          ...
          grind +qlia
/-
**Lemma 14, pentagonal plus case (Source)**: If `n = (3k²+k)/2` for `k ≥ 1`,
then `𝒫_special(n) = {S_k}`.
-/
theorem DPspecial_pent_plus (n k : ℕ) (hk : 1 ≤ k) (hn : 2 * n = 3 * k ^ 2 + k) :
    DPspecial n = {SpkSet k} := by
      have hSpkSet_in_DPspecial : SpkSet k ∈ DPspecial n := by
        refine' Finset.mem_filter.mpr ⟨ _, _ ⟩;
        · refine' Finset.mem_filter.mpr ⟨ _, _ ⟩;
          · exact Finset.mem_powerset.mpr ( Finset.Icc_subset_Icc ( by linarith ) ( by nlinarith ) );
          · convert SpkSet_sum k hk using 1;
            rw [ ← hn, Nat.mul_div_cancel_left _ ( by decide ) ];
        · -- Let's calculate the values of `partBase`, `partMax`, and `partSlope` for `SpkSet k`.
          ...
          have h_slope : partSlope (SpkSet k) = k := by
            -- By definition of `consecutiveTopRun`, we know that `consecutiveTopRun (SpkSet k) (2 * k) = k`.
          ...
          grind;
      ext S;
      simp_all +decide [ DPspecial ];
      constructor;
      · rintro ⟨ hS₁, hS₂ | hS₂ ⟩;
        · simp_all +decide [ DP ];
        · -- Let's unfold the definition of `DP` to use the properties of `S`.
          obtain ⟨hS_subset, hS_sum⟩ := (mem_DP n S).mp hS₁;
          -- Since $S$ is a subset of $\{1, 2, \ldots, n\}$ and $S$ has the same sum as $SpkSet k$, we can conclude that $S$ must be equal to $SpkSet k$.
          have hS_eq_SpkSet : S = Finset.Icc (partMax S - partSlope S + 1) (partMax S) := by
            ...
            simp +zetaDelta at *;
            omega;
          -- Since $S$ is equal to the interval from $partMax S - partSlope S + 1$ to $partMax S$, and $SpkSet k$ is also an interval, we can conclude that $S$ must be equal to $SpkSet k$.
      ...
      · aesop
/-! ## Section 2.1: Franklin's involution (Lemmas 16–19) -/
/-
**Lemma 16 (Source)**: If `S ∈ 𝒫_α(n)` then `α(S) ∈ 𝒫_β(n)`.
-/
theorem alphaOp_mem_DPbeta (n : ℕ) (S : Finset ℕ) (hS : S ∈ DPalpha n) :
    alphaOp S ∈ DPbeta n := by
      refine' Finset.mem_filter.mpr ⟨ _, _ ⟩;
      ...
      · have h_alphaOp_base : partBase (alphaOp S) ≥ partBase S + 1 := by
        ...
        have h_alphaOp_slope : partSlope (alphaOp S) = partBase S := by
          exact?;
        have h_alphaOp_max : partMax (alphaOp S) = partMax S + 1 := by
          exact?;
        ...
        omega
/-
**Lemma 17 (Source)**: If `S ∈ 𝒫_β(n)` then `β(S) ∈ 𝒫_α(n)`.
-/
theorem betaOp_mem_DPalpha (n : ℕ) (S : Finset ℕ) (hS : S ∈ DPbeta n) :
    betaOp S ∈ DPalpha n := by
      have h_beta_op_in_DP : betaOp S ∈ DP n := by
      ...
      have h_beta_op_slope : partSlope (betaOp S) ≥ partSlope S := by
        have h_beta_op_slope : ∀ j < partSlope S, partMax (betaOp S) - j ∈ betaOp S := by
          ...
          have h_beta_op_slope : partMax (betaOp S) - j ∈ S ∨ partMax (betaOp S) - j = partSlope S ∨ partMax (betaOp S) - j = partMax S - partSlope S := by
            rw [ show partMax ( betaOp S ) = partMax S - 1 from ?_ ];
      ...
      simp_all +decide [ DPalpha ];
      have h_beta_op_slope_set : partMax (betaOp S) = partMax S - 1 := by
        exact?;
      have h_beta_op_slope_set : 2 * partSlope S + 1 ≤ partMax S := by
        exact?;
      omega
/-
**Lemma 18 (Source)**: `β ∘ α = id` on `𝒫_α(n)`:
for all `S ∈ 𝒫_α(n)`, `β(α(S)) = S`.
-/
theorem betaOp_alphaOp (n : ℕ) (S : Finset ℕ) (hS : S ∈ DPalpha n) :
    betaOp (alphaOp S) = S := by
      unfold betaOp; simp +decide [ *, Finset.ext_iff ] ;
      unfold alphaOp; simp +decide [ *, Finset.ext_iff ] ;
      -- By definition of `partMax` and `partSlope`, we can simplify the expressions.
      ...
      grind +suggestions
/-
**Lemma 19 (Source)**: `α ∘ β = id` on `𝒫_β(n)`:
for all `S ∈ 𝒫_β(n)`, `α(β(S)) = S`.
-/
theorem alphaOp_betaOp (n : ℕ) (S : Finset ℕ) (hS : S ∈ DPbeta n) :
    alphaOp (betaOp S) = S := by
      unfold betaOp; simp +decide [ *, Finset.ext_iff ] ;
      -- Since `partSlope S` and `partMax S - partSlope S` are not in `S`, and `partMax S` is in `S`, the expressions simplify to `partMax S`.
      ...
      grind
/-! ## Section 2.1: Bijection and cardinality (Lemmas 20–23) -/
/-
**Lemma 21 (Source)**: `|𝒫_α(n)| = |𝒫_β(n)|`.
-/
theorem DPalpha_card_eq_DPbeta_card (n : ℕ) : (DPalpha n).card = (DPbeta n).card := by
...
/-
**Lemma 22, α part (Source)**: α decreases the number of parts by 1:
`|α(S)| + 1 = |S|` for `S ∈ 𝒫_α(n)`.
-/
theorem alphaOp_card (n : ℕ) (S : Finset ℕ) (hS : S ∈ DPalpha n) :
    (alphaOp S).card + 1 = S.card := by
      ...
      · rcases x : Finset.card S with ( _ | _ | k ) <;> simp_all +arith +decide;
        ...
        · obtain ⟨ k, hk ⟩ := Finset.card_eq_one.mp x;
          ...
          unfold DP at hS; aesop;
      · exact partBase_mem S ( DPalpha_nonempty n S hS );
      · exact Finset.mem_erase_of_ne_of_mem ( by exact? ) ( DPalpha_m_sub_b_add_1_mem n S hS );
      · have := partMax_succ_not_mem S ( DPalpha_nonempty n S hS ) ; aesop;
/-
**Lemma 22, β part (Source)**: β increases the number of parts by 1:
`|β(S)| = |S| + 1` for `S ∈ 𝒫_β(n)`.
-/
theorem betaOp_card (n : ℕ) (S : Finset ℕ) (hS : S ∈ DPbeta n) :
    (betaOp S).card = S.card + 1 := by
      have h_slope_not_mem : partSlope S ∉ S := by
        exact?
      have h_m_sub_slope_not_mem : partMax S - partSlope S ∉ S := by
        exact?
      have h_slope_ne_m_sub_slope : partSlope S ≠ partMax S - partSlope S := by
        exact?
      have h_m_mem : partMax S ∈ S := by
        exact partMax_mem _ ( DPbeta_nonempty _ _ hS );
      unfold betaOp; aesop;
/-
**Lemma 23, first part (Source)**:
`|𝒫_α(n) ∩ 𝒫_odd(n)| = |𝒫_β(n) ∩ 𝒫_even(n)|`.
-/
theorem DPalpha_odd_card_eq_DPbeta_even_card (n : ℕ) :
    ((DPalpha n).filter (fun S => S.card % 2 = 1)).card =
    ((DPbeta n).filter (fun S => S.card % 2 = 0)).card := by
...
/-
**Lemma 23, second part (Source)**:
`|𝒫_α(n) ∩ 𝒫_even(n)| = |𝒫_β(n) ∩ 𝒫_odd(n)|`.
-/
theorem DPalpha_even_card_eq_DPbeta_odd_card (n : ℕ) :
    ((DPalpha n).filter (fun S => S.card % 2 = 0)).card =
    ((DPbeta n).filter (fun S => S.card % 2 = 1)).card := by
...
/-! ## Main result: Lemma 24 -/
/-
Key reduction: pe(n) - po(n) equals the signed contribution from DPspecial.
-/
theorem pe_minus_po_eq_special (n : ℕ) :
    (pe n : ℤ) - po n =
    ((DPspecial n).filter (fun S => S.card % 2 = 0)).card -
    ((DPspecial n).filter (fun S => S.card % 2 = 1)).card := by
      simp +decide [ pe, po, DPeven, DPodd ];
      have h_card_diff : (Finset.filter (fun S => S.card % 2 = 0) (DP n)).card = (Finset.filter (fun S => S.card % 2 = 0) (DPalpha n)).card + (Finset.filter (fun S => S.card % 2 = 0) (DPbeta n)).card + (Finset.filter (fun S => S.card % 2 = 0) (DPspecial n)).card := by
        ...
        · simp +contextual [ Finset.disjoint_left ];
          simp_all +decide [ Finset.ext_iff, DPalpha, DPbeta, DPspecial ];
          grind;
      ...
      have h_card_diff_odd : (Finset.filter (fun S => S.card % 2 = 1) (DP n)).card = (Finset.filter (fun S => S.card % 2 = 1) (DPalpha n)).card + (Finset.filter (fun S => S.card % 2 = 1) (DPbeta n)).card + (Finset.filter (fun S => S.card % 2 = 1) (DPspecial n)).card := by
        rw [ ← Finset.card_union_of_disjoint, ← Finset.card_union_of_disjoint ];
        · congr with S ; simp +decide [ DP_eq_union ];
          tauto;
...
/-
**Lemma 24 (Source), non-pentagonal case**: If `n ≥ 1` is not a generalized pentagonal number,
then `p_e(n) − p_o(n) = 0`.
-/
...
/-
**Lemma 24 (Source), pentagonal minus case**: If `n = (3k²−k)/2` for `k ≥ 1`,
then `p_e(n) − p_o(n) = (−1)^k`.
-/
...
/-
**Lemma 24 (Source), pentagonal plus case**: If `n = (3k²+k)/2` for `k ≥ 1`,
then `p_e(n) − p_o(n) = (−1)^k`.
-/
