...
theorem summable_S_summand {q : ℂ} (hq : ‖q‖ < 1) (k : ℕ) :
  ...
  have h_summable : Summable (fun m : ℕ => (q ^ (m * (m + k))) / (qPochhammer q q m * qPochhammer q q (m + k))) := by
    have h_bound : ∃ C > 0, ∀ m : ℕ, ‖qPochhammer q q m * qPochhammer q q (m + k)‖ ≥ C := by
      -- Since $|q| < 1$, the product $\prod_{j=0}^{m-1} (1 - q^j)$ converges to a nonzero value as $m \to \infty$.
      have h_prod_nonzero : ∃ C > 0, ∀ m : ℕ, ‖qPochhammer q q m‖ ≥ C := by
        -- The infinite product $\prod_{j=0}^{\infty} (1 - q^j)$ converges to a nonzero value since $|q| < 1$.
...
/-
Limit: S_k → 1/(q;q)_∞ as k → ∞.
-/
theorem S_sum_tendsto {q : ℂ} (hq : ‖q‖ < 1) :
    Tendsto (S_sum q) atTop (𝓝 (1 / qPochhammerInf q q)) := by
  -- The m=0 term is 1/((q;q)_0 (q;q)_k) = 1/(q;q)_k → 1/(q;q)_∞ by tendsto_qPochhammer.
  ...
  -- For m ≥ 1, |q>Pochhammer q q m| ≥ C > 0, so |q^(m(m+k)) / ((q;q)_m (q;q)_{m+k})| ≤ |q|^(m(m+k)) / C ≤ |q|^m / C.
  have h_bound : ∃ C > 0, ∀ m ≥ 1, ∀ k : ℕ, ‖qPochhammer q q m * qPochhammer q q (m + k)‖ ≥ C := by
    -- By definition of $qPochhammer$, we know that $|qPochhammer q q m| \geq C$ for some $C > 0$ and all $m \geq 1$.
  ...
  -- By the dominated convergence theorem, the sum of the remaining terms (m ≥ 1) tends to 0.
    ...
    have h_split : ∑' m : ℕ, q ^ (m * (m + k)) * (1 - q ^ m) / (qPochhammer q q m * qPochhammer q q (m + k + 1)) = ∑' m : ℕ, q ^ ((m + 1) * (m + 1 + k)) / (qPochhammer q q m * qPochhammer q q (m + k + 2)) := by
      ...
      · have h_summable : Summable (fun m : ℕ => q ^ (m * (m + k)) / (qPochhammer q q m * qPochhammer q q (m + k + 1))) := by
          ...
          have h_summable : Summable (fun m : ℕ => q ^ (m * (m + k)) / (qPochhammer q q m * qPochhammer q q (m + k)) * (1 / (1 - q ^ (m + k + 1)))) := by
            ...
            -- Since the norm of the second term is bounded by C, and the first term is summable, their product is also summable.
            ...
            -- Apply the comparison test to conclude that the original series is summable.
            ...
            -- Apply the comparison test to conclude that the original series is summable using the fact that the norm of each term is bounded by a summable series.
...
/-
The main result: S_k = 1/(q;q)_∞ for all k.
-/
theorem S_sum_eq {q : ℂ} (hq : ‖q‖ < 1) (k : ℕ) :
    S_sum q k = 1 / qPochhammerInf q q := by
  -- By induction, we can show that $S_k - S_{k+1} = 0$ for all $k$.
  have h_ind : ∀ k : ℕ, S_sum q k - S_sum q (k + 1) = 0 := by
    intro k;
    -- By induction on $n$, we can show that $|D_k| \leq \|q\|^{n(2k+n+1)/2} |D_{k+n}|$.
    ...
    -- Since $S_k \to 1/(q;q)_\infty$ and $S_{k+n} \to 1/(q;q)_\infty$, we have $S_{k+n} - S_{k+n+1} \to 0$.
    ...
    -- Since ‖q‖ < 1, we have ‖q‖ ^ (n * (2 * k + n + 1) / 2) → 0 as n → ∞.
  ...
  -- Since $S_k = S_{k+1}$ for all $k$, we have $S_k = S_0$ for all $k$.
...
/-
The coefficient of z^k in the Cauchy product of the two Euler expansions:
    Σ_m c_{m+k} d_m = q^{C(k,2)} for k ≥ 0.
-/
...
/-
The coefficient of z^{-l} in the Cauchy product:
    Σ_n c_n d_{n+l} = q^{C(l+1,2)} for l ≥ 1.
-/
theorem cauchy_coeff_neg {q : ℂ} (hq : ‖q‖ < 1) (l : ℕ) :
    HasSum (fun n : ℕ =>
      ...
      (q ^ (l + 2).choose 2) := by
  -- Factor out the common terms and simplify the sum.
